...
type MessageFormatOptions = {
	readonly commandHide?: boolean;
	readonly imageHide?: boolean;
	readonly offTopicHide?: boolean;
...
const bracketBase64ImageRe = new RegExp(
	`\\[(?:image|图):base64://${base64BodyRe}\\]`,
	"g",
);

function inferBase64ImageMime(base64: string): string {
...
export function base64ImageSrc(base64: string): string {
	const compact = base64.replaceAll(/\s+/g, "");
	return `data:${inferBase64ImageMime(compact)};base64,${compact}`;
}
...
export function msgImageFormat(
	msg: string,
	...
	if (options.imageHide) {
		msg = msg.replaceAll(cqMarkdownImageUrlRe, "");
		msg = msg.replaceAll(cqBase64ImageRe, "");
		msg = msg.replaceAll(
		...
		if (htmlText) {
			msg = msg.replaceAll(cqBase64ImageRe, (_match, base64: string) => {
				return `<img style="max-width: min(300px, 100%); height: auto" src="${base64ImageSrc(base64)}" />`;
			});
			msg = msg.replaceAll(
			...
			msg = msg.replaceAll(
				...
				'<img style="max-width: min(300px, 100%); height: auto" src="https://gchat.qpic.cn/gchatpic_new/0/0-0-$2/0?term=2" crossorigin="anonymous" />',
			);
			...
			msg = msg.replaceAll(
				...
				'<img style="max-width: min(300px, 100%); height: auto" src="https://gchat.qpic.cn/gchatpic_new/0/0-0-$1/0?term=2,subType=1" />',
			);
			...
			msg = msg.replaceAll(
				...
				(_m, p1) => {
					return `<img style="max-width: min(300px, 100%); height: auto" src="https://gchat.qpic.cn/gchatpic_new/0/0-0-${p1.toUpperCase()}/0?term=2,subType=1" />`;
				},
			);
			msg = msg.replaceAll(
				/\[CQ:image,file=\{([A-Z0-9]+)-([A-Z0-9]+)-([A-Z0-9]+)-([A-Z0-9]+)-([A-Z0-9]+)}([^\]]+?)\]/g,
				'<img style="max-width: min(300px, 100%); height: auto" src="https://gchat.qpic.cn/gchatpic_new/0/0-0-$1$2$3$4$5/0?term=2" />',
			);
		}
	}
	...
	if (options.imageHide) {
		msg = msg.replaceAll(bracketBase64ImageRe, "");
		msg = msg.replaceAll(/\[(image|图):[^\]]+\]/g, "");
	} else {
		if (htmlText) {
			msg = msg.replaceAll(
				bracketBase64ImageRe,
				(_match, base64: string) =>
					`<img style="max-width: min(300px, 100%); height: auto" src="${base64ImageSrc(base64)}" />`,
			);
			msg = msg.replaceAll(
				/\[(?:image|图):([^\]]+)?([^\]]+)\]/g,
				'<img style="max-width: min(300px, 100%); height: auto" src="$1" />',
...
export function msgOffTopicFormat(
	msg: string,
	options: MessageFormatOptions,
	isDice = false,
) {
...
export function msgCommandFormat(msg: string, options: MessageFormatOptions) {
	// 替换指令
...
export function msgIMUseridFormat(
	msg: string,
	options: MessageFormatOptions,
	isDice = false,
) {
...
export function msgAtFormat(msg: string, pcList: readonly MentionPcItem[]) {
	// qq的回复会给出两个连续的相同CQ码，如[CQ:at,qq=123456] [CQ:at,qq=123456]，预先处理只保留一个
	const qqReplyPattern =
		/((\[CQ:at,[^\]]*qq=([0-9]+)[^\]]*\])) \[CQ:at,[^\]]*qq=\3[^\]]*\]/g;
	...
	const safeMention = (value: string) => {
	...
	msg = msg.replace(/\[CQ:at,([^\]]+)\]/g, (_match, attrStr) => {
		...
		if (mappedName) {
			return safeMention(mappedName);
		}
		const nameAttr = attrMap.get("name");
		if (nameAttr) {
			const decoded = decodeHTML(nameAttr);
			return safeMention(decoded);
		}
		if (qq) {
			return safeMention(qq);
		}
		return "@未知用户";
	});
	...
	for (const i of pcList) {
		// discord的回复是 <@8181007086111111>
		const discordAtPattern = new RegExp(`&lt;@${i.IMUserId}&gt;`, "g");
		msg = msg.replace(discordAtPattern, safeMention(i.name));

		// kook的回复是 (met)17603111(met)
		const kookAtPattern = new RegExp(`\\(met\\)${i.IMUserId}\\(met\\)`, "g");
		msg = msg.replace(kookAtPattern, safeMention(i.name));
	}
	return msg;
}
