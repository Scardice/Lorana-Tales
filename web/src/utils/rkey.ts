...
function replaceMessage(message: string, payload: RKeyPayload) {
	...
	const cqURLRe =
		/(url=|file=)(https?:\/\/(?:multimedia\.nt\.qq\.com\.cn|gchat\.qpic\.cn)[^,\]\s]+)/g;
	updated = updated.replace(cqURLRe, (_m, prefix: string, url: string) => {
		return `${prefix}${updateURL(url, payload)}`;
	});

	const bracketRe =
		/(\[(?:image|图):)(https?:\/\/(?:multimedia\.nt\.qq\.com\.cn|gchat\.qpic\.cn)[^\]\s]+)(\])/g;
	updated = updated.replace(
		bracketRe,
		(_m, prefix: string, url: string, suffix: string) => {
...
export function shouldApplyQQImageRKeyReplacement(text: string) {
	return (
		text.includes(newImageHost) ||
		text.includes("appid=1406") ||
		text.includes("appid=1407")
	);
}

export function applyQQImageRKeyReplacement(
	text: string,
	payload: RKeyPayload,
) {
