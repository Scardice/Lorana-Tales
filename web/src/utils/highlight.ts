...
export function generateLang(
	...
	const tagNameLine = Tag.define();
	const tagNameLineHost = Tag.define();
	const tagNameLineDice = Tag.define();
	const tagNameLineHide = Tag.define();
		...
		token(stream, state) {
			...
			switch (state.mode) {
				...
				case 1: {
					...
					m = stream.match(
						/^\[CQ:image[^\]]*?file=base64:\/\/[A-Za-z0-9+/=\s]+\]/g,
					) as RegExpMatchArray;
					if (m) {
						state.text = "";
						return `image-${state.name}`;
	...
	const myHighlightStyle = syntaxHighlighting(
		HighlightStyle.define(highLights),
	);

	return [new LanguageSupport(language, [exampleCompletion]), myHighlightStyle];
}
