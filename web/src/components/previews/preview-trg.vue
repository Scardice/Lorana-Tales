<template>
  <div style="margin-bottom: 0.5rem" v-show="isShow">
    <div style="text-align: center; margin-bottom: 2rem; margin-top: 0.5rem">
      <div>
        ...
        <n-button
          ...
          tag="a"
          target="_blank"
          href="https://github.com/DanDDXuanX/TRPG-Replay-Generator"
          >回声工坊</n-button
        >可以将余烬的log一键转视频哦！
      </div>
      <div>
        回声工坊的介绍和视频教程看这里：<n-button
          type="primary"
          text
          tag="a"
          target="_blank"
          href="https://www.bilibili.com/video/BV1PC4y1j7P2/"
          >B站传送门</n-button
        >
      </div>
    </div>
    ...
    <n-checkbox
      label="添加语音合成标记"
      v-model:checked="store.trgIsAddVoiceMark"
    />
  </div>
  ...
  <div class="preview" ref="preview" id="preview" v-show="isShow">
    ...
    <div v-if="previewItems.length === 0">
      ...
      <div>
        已知支持的格式有:
        余烬Log(json)、海豹Log(json)、赵/Dice!原始文件、塔原始文件
      </div>
