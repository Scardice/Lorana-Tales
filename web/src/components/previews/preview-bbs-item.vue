<template>
  <div class="">
    <span style="color: #aaa" class="_time" v-if="!store.exportOptions.timeHide"
      >[color={{ getTimeColor() }}]{{ timeSolve(source) }}[/color]</span
    >
    <span :style="{ color: colorByName(source) }"
      >[color={{ colorByName(source) }}]
  ...
  if (store.bbsUseSpaceWithMultiLine) {
    ...
    return msg
      ...
      .replaceAll(
        "\n",
        '<br/><span class="lf">\n</span>' +
          (!store.exportOptions.timeHide
            ? `<span style='color:#aaa'>${toSpace(timeSolve(i))}</span>`
            : "&ensp;") +
          escapeHTML(nicknameSolve(i)),
      );
  ...
  return msg
    ...
    .replaceAll(
      ...
      '[/color]<br/><span class="lf">\n</span>' +
        (!store.exportOptions.timeHide
          ? `<span style='color:#aaa'>[color=${getTimeColor()}]${timeSolve(i)}[/color]</span>`
          : "") +
        `[color=${colorByName(i)}] ` +
        escapeHTML(nicknameSolve(i)),
    );
