<template>
  <div class="">
    <div :style="source.isDice ? 'margin-top: 16px; margin-bottom: 16px' : ''">
      <span :style="{ color: colorByName(source) }" v-if="source.isDice"
        >#
      </span>
...
type CommandInfoRow = Record<string, unknown>;

type TrgCommandInfo = {
	rule?: string;
	cmd?: string;
	pcName?: string;
	items?: CommandInfoRow[];
};

function readCommandInfo(value: unknown): TrgCommandInfo | undefined {
	if (typeof value !== "object" || value === null) return undefined;
	const record = value as Record<string, unknown>;
	return {
		rule: typeof record.rule === "string" ? record.rule : undefined,
		cmd: typeof record.cmd === "string" ? record.cmd : undefined,
		pcName: typeof record.pcName === "string" ? record.pcName : undefined,
		items: Array.isArray(record.items)
			? (record.items as CommandInfoRow[])
			: [],
	};
}

function firstExpr(row: CommandInfoRow): string {
	const exprs = row.exprs;
	return Array.isArray(exprs) ? String(exprs[0] || "") : "";
}
...
const nicknameSolve = (i: LogItem) => {
	const _options = store.exportOptions;
...
const readDiceNum = (expr: string, defaultVal = 100) => {
	...
	if (m) {
		diceNum = parseInt(m[1], 10);
	}
	return diceNum;
};

const trgCommandSolve = (item: LogItem) => {
	const ci = readCommandInfo(item.commandInfo);
	if (ci) {
		const ciItems = ci.items ?? [];
		if (ci.rule === "coc7") {
			switch (ci.cmd) {
				case "ra": {
					let items = [];
					for (let i of ciItems) {
						let diceNum = readDiceNum(String(i.expr1 || ""));
						if (i.version === 101) {
							items.push(
								`(${ci.pcName}的${i.expr2},${diceNum},${i.checkVal},${i.outcome})`,
							);
						} else {
							items.push(
								`(${ci.pcName}的${i.expr2},${diceNum},${i.attrVal},${i.checkVal})`,
							);
						}
					}
				...
				case "st": {
					...
					for (let i of ciItems) {
						if (i.attr === "hp") {
							let maxNow = Math.max(Number(i.valOld), Number(i.valNew));
							items.push(
								`<hitpoint>:(${ci.pcName},${maxNow},${i.valOld},${i.valNew})`,
							);
						}
				...
				case "sc": {
					...
					let items = [];
					for (let i of ciItems) {
						const expr = firstExpr(i);
						let diceNum = readDiceNum(expr);
						items.push(
							`(${ci.pcName}的${expr},${diceNum},${i.sanOld},${i.outcome ?? i.checkVal})`,
						);
					}
					return `<dice>:${items.join(",")}`;
				}
		...
		if (ci.rule === "dnd5e") {
			switch (ci.cmd) {
				case "st": {
					...
					let tip = "";
					if (hasHp) {
						let _tip = "# 请注意，当前版本需要手动调整下方最大生命值(第二项)\n";
					}
					return `${tip}${items.join("\n")}`;
				}
				case "rc": {
					...
					let tip = "";
					for (let i of ciItems) {
						let diceNum = readDiceNum(String(i.expr || ""), 20);
						items.push(
							`(${ci.pcName}的${i.reason}检定,${diceNum},NA,${i.result})`,
						);
						tip =
							"# 请注意，DND的最大面数可能为 D20+各种加值，需要手动二次调整\n";
					}
		...
		switch (ci.cmd) {
			case "roll": {
				...
				let items = [];
				for (let i of ciItems) {
					let diceNum = readDiceNum(String(i.expr || ""));
