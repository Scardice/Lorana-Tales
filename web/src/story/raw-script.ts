...
import type { StoryArchive, StoryCharacter, StoryEffectColor, StoryInteractionEffect, StoryMessage, StoryPersistentEffect, StoryScreenEffect, StorySettings, StoryStreamTokenAnimation } from "./types";
...
export function parseStoryScript(script: string, base: StoryArchive): StoryArchive {
  const lines = script.replace(/\r\n?/g, "\n").split("\n"); const document = structuredClone(base.document); document.characters = []; document.messages = []; document.effectTracks = []; let declaredUpdatedAt = "";
...
• screen="damage" screen-color="red"：形态与颜色彼此独立；颜色仅支持内置安全调色盘。
