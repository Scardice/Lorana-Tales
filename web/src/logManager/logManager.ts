...
import type { LogImporter, TextInfo } from "./importers/_logImpoter";
...
import { OlivaDiceLogImporter } from "./importers/OlivaDiceLogImporter";
...
import { ScardiceLogImporter } from "./importers/ScardiceLogImporter";
...
export class LogManager {
  ...
  importers = [
    ["scardice", new ScardiceLogImporter(this)],
    ["olivaDice", new OlivaDiceLogImporter(this)],
  ...
  parse(text: string, genFakeHeadItem: boolean = false) {
    for (const [_theName, _importer] of this.importers) {
  ...
  syncChange(curText: string, r1: number[], r2: number[]) {
    ...
    if (!this.lastText) {
      ...
      const influence = [];
      const [a, b] = r1;
      let last: indexInfoListItem | undefined;
      ...
      if (influence.length) {
        ...
        const right = this.lastIndexInfoList.slice(
          riIndex + 1,
          this.lastIndexInfoList.length,
        );

        const rInfo = this.parse(
          changedText,
        ...
        if (rInfo) {
          ...
          const ret = this.exporters.editLog.doExport(
            rInfo.items,
            li.indexStart,
          );

          if (ret) {
            const { indexInfoList } = ret;
        ...
        // 封装最终文本
        const newText =
          this.lastText.slice(0, li.indexStart) +
          changedText +
          this.lastText.slice(ri.indexEnd, this.lastText.length);
...
setTimeout(() => {
  Reflect.set(globalThis, "logMan", logMan);
}, 1000);
