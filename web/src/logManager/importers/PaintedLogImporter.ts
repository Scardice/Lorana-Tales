...
export class PaintedLogImporter extends LogImporter {
	...
	parse(text: string) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			// 匹配<角色名>消息格式
			const angleMatch = rePaintedLogAngleBrackets.exec(line);
			// 匹配[角色名]消息格式
			const squareMatch = rePaintedLogSquareBrackets.exec(line);
			// 匹配带日期时间的格式
			const dateTimeMatch = rePaintedLogWithDateTime.exec(line);
