			...
			<n-modal-provider>
				<n-notification-provider>
					<div v-if="ready" class="editor-router" :class="[playbackRoute ? 'editor-router--playback' : `editor-router--${requestedMode}`, isDark ? 'theme-dark' : 'theme-light', { 'editor-router--upper-collapsed': upperBarCollapsed }]">
						...
						<Teleport to="body"><Transition name="mode-warning"><div v-if="firstTutorialPromptOpen" class="anchored-hint-layer first-tutorial-prompt"><section role="dialog" aria-labelledby="first-tutorial-title"><header><div><strong id="first-tutorial-title">第一次使用 Lorana Tales 吗？</strong><small>两分钟认识编辑器</small></div></header><p>要不要先看“基本使用教程”？以后也可以随时从输入区设置或顶部更多菜单再次打开。</p><label><input v-model="firstTutorialPromptNeverShow" type="checkbox" />以后不再显示</label><footer><button type="button" @click="answerFirstTutorial(false)">暂时不用</button><button class="primary" type="button" @click="answerFirstTutorial(true)">看看教程</button></footer></section></div></Transition></Teleport>
...
.mode-toggle-wrap{position:relative}.anchored-hint{position:absolute;z-index:22000;display:grid;width:min(340px,calc(100vw - 1.5rem));gap:.65rem;border:1px solid var(--control-border);border-radius:13px;padding:.8rem;background:var(--panel-surface);box-shadow:0 16px 48px #0008;color:var(--control-text)}.anchored-hint::before{position:absolute;width:12px;height:12px;border-top:1px solid var(--control-border);border-left:1px solid var(--control-border);background:var(--panel-surface);content:"";transform:rotate(45deg)}.anchored-hint--mode{top:calc(100% + .7rem);left:0}.anchored-hint--mode::before{top:-7px;left:18px}.anchored-hint p{margin:0;color:var(--muted-text);line-height:1.55}.anchored-hint label,.anchored-hint-layer label{display:flex;align-items:center;gap:.4rem;color:var(--muted-text);font-size:.82rem}.anchored-hint button{justify-self:end;min-height:34px;border:1px solid var(--primary-bg);border-radius:8px;padding:.35rem .7rem;background:var(--primary-bg);color:var(--primary-text)}.anchored-hint-layer{position:fixed;z-index:22000;right:12px;bottom:76px;width:min(420px,calc(100vw - 24px));pointer-events:none}.anchored-hint-layer section{position:relative;box-sizing:border-box;display:grid;gap:.65rem;border:1px solid var(--control-border);border-radius:14px;padding:.9rem;background:var(--panel-surface);box-shadow:0 18px 55px #0008;color:var(--control-text);pointer-events:auto}.anchored-hint-layer section::after{position:absolute;right:22px;bottom:-7px;width:12px;height:12px;border-right:1px solid var(--control-border);border-bottom:1px solid var(--control-border);background:var(--panel-surface);content:"";transform:rotate(45deg)}.anchored-hint-layer header>div{display:grid}.anchored-hint-layer header small,.anchored-hint-layer p{color:var(--muted-text)}.anchored-hint-layer p{margin:0;line-height:1.5}.anchored-hint-layer footer{display:flex;justify-content:flex-end;gap:.5rem}.anchored-hint-layer button{min-height:35px;border:1px solid var(--control-border);border-radius:8px;padding:.4rem .7rem;background:var(--control-surface);color:var(--control-text)}.anchored-hint-layer button.primary{border-color:var(--primary-bg);background:var(--primary-bg);color:var(--primary-text)}@media(max-width:650px){.anchored-hint--mode{position:fixed;top:52px;left:8px}.anchored-hint-layer{right:8px;bottom:70px;width:calc(100vw - 16px)}}
</style>
